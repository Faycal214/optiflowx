...
class BoxJenkinsIdentificationResult:
    """Auditable Stage 9 identification output.

    ``candidate_orders`` contains ``(p, d, q)`` tuples only. It is candidate
    generation, not model selection: no tuple is labelled as selected here.
    """
    ...
    def ACF(self) -> ACFResult:
        """Backward-compatible access to the identification ACF result."""
        return self.acf_result

    @property
    def PACF(self) -> PACFResult:
        """Backward-compatible access to the identification PACF result."""
        return self.pacf_result

    @property
    def d(self) -> int:
        """Return the selected differencing order for candidate generation."""
        return self.differencing_order

    def table(self) -> pd.DataFrame:
        """Return a deterministic candidate-order table."""
        return pd.DataFrame(
...
def _choose_differencing(
    ...
    *,
    ...
    alpha: float,
    whitening_lags: int,
) -> tuple[int, np.ndarray, tuple[str, ...], str]:
    if d is not None:
        if not isinstance(d, int) or isinstance(d, bool) or d < 0 or d > max_d:
            raise ValueError("d must be an integer in [0, max_d]")
        transformed = _difference(values, d)
        return d, transformed, (f"d={d}: explicitly supplied by caller",), "explicit differencing"

    decisions: list[str] = []
    transformed = values.copy()
    for current_d in range(max_d + 1):
        result = dickey_fuller_sequential(
            transformed,
            max_lags=None,
            autolag=None,
            alpha=alpha,
        ...
        decisions.append(
            f"d={current_d}: {result.selected.decision}; nature={result.nature}; common_lag={result.common_lag}"
        )
        if result.selected.rejects_null or current_d == max_d:
            rule = "smallest d<=max_d for which the course-faithful sequential DF test rejects the unit-root null"
            return current_d, transformed, tuple(decisions), rule
...
def _initial_contiguous_significant(result, *, max_order: int) -> tuple[int, tuple[int, ...]]:
    ...
    hint = 0
    for lag in range(1, min(max_order, result.nlags) + 1):
        if significant[lag]:
            hint = lag
        else:
            break
...
def _candidate_orders(
    ...
    *,
    ...
    candidates = {
        (0, d, 0),
        *((p, d, 0) for p in range(1, p_cap + 1)),
        *((0, d, q) for q in range(1, q_cap + 1)),
        *((p, d, q) for p, q in product(range(1, p_cap + 1), range(1, q_cap + 1))),
    }
    return tuple(sorted(candidates))


def identify_box_jenkins(
    y,
    *,
    d: int | None = None,
    max_d: int = 2,
...
) -> BoxJenkinsIdentificationResult:
    """Perform deterministic Stage 9 Box-Jenkins identification.

    Automatic differencing chooses the smallest order up to ``max_d`` whose
    course-faithful sequential Dickey-Fuller test rejects the unit-root null.
    ACF/PACF are then computed with the frozen Stage 8 numerical primitives.
    The AR/MA hints are the initial contiguous runs of significant PACF/ACF
    lags, and the candidate set is the deterministic Cartesian product up to
    bounded hint orders, with a low-order fallback when no cutoff is detected.
    """
    ...
    d_order, transformed, decisions, rule = _choose_differencing(
        original,
        d=d,
        max_d=max_d,
        alpha=alpha,
    ...
    interpretation = (
        f"After d={d_order} differencing, PACF significant lags={list(pc_sig)} and "
        f"ACF significant lags={list(ac_sig)}. Initial contiguous PACF cutoff={ar_hint} "
        f"suggests AR({ar_hint}) and ACF cutoff={ma_hint} suggests MA({ma_hint}). "
        f"Generated {len(candidates)} deterministic candidate orders; no model is selected at this stage."
    )
    return BoxJenkinsIdentificationResult(
        original=original,
        transformed=transformed,
        differencing_order=d_order,
        differencing_rule=rule,
        stationarity_decisions=decisions,
        acf_result=ac,
        pacf_result=pc,
        acf_significant_lags=ac_sig,
        pacf_significant_lags=pc_sig,
        ar_order_hint=ar_hint,
        ma_order_hint=ma_hint,
...
def identify(y, *, nlags: int = 24, alpha: float = 0.05) -> dict[str, object]:
    """Return the legacy ACF/PACF tables and simple AR/MA order clues."""
