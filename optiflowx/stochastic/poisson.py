...
class PoissonProcess:
    ...
    def count_probability(self, n: int, t: float) -> float:
        self._validate_time(t); self._validate_count(n)
        x = self.rate * float(t)
    ...
    def conditional_arrival_times(self, k: int, s: float, *, rng: np.random.Generator | None = None) -> np.ndarray:
        self._validate_positive_count(k); self._validate_positive_time(s, "s")
        generator = rng if rng is not None else np.random.default_rng()
        return np.sort(generator.uniform(0.0, float(s), size=int(k)))

    def superpose(self, other: "PoissonProcess") -> "PoissonProcess":
        if not isinstance(other, PoissonProcess): raise TypeError("other must be a PoissonProcess")
    ...
    def _validate_count(n: int) -> None:
        if isinstance(n, bool) or not isinstance(n, (int, np.integer)) or n < 0: raise ValueError("n must be a non-negative integer")

    @staticmethod
    def _validate_positive_count(n: int) -> None:
        if isinstance(n, bool) or not isinstance(n, (int, np.integer)) or n < 1: raise ValueError("n must be a positive integer")

    @staticmethod
    def _validate_time(t: float) -> None:
        if not np.isfinite(t) or t < 0: raise ValueError("time must be finite and non-negative")

    @staticmethod
    def _validate_positive_time(t: float, name: str) -> None:
        if not np.isfinite(t) or t <= 0: raise ValueError(f"{name} must be strictly positive and finite")

    @staticmethod
    def _validate_interval(s: float, t: float) -> None:
        PoissonProcess._validate_time(s); PoissonProcess._validate_time(t)
        if s > t: raise ValueError("require s <= t")
...
class NonHomogeneousPoissonProcess:
    ...
    def __init__(self, intensity: Callable[[float], float], mean_function: Callable[[float], float] | None = None) -> None:
        if not callable(intensity): raise TypeError("intensity must be callable")
        self.intensity = intensity; self.mean_function = mean_function
    ...
    def mean(self, t: float) -> float:
        ...
        if self.mean_function is not None:
            ...
            if not np.all(np.isfinite(values)) or np.any(values < 0): raise ValueError("intensity must return finite non-negative values")
            value = float(np.trapezoid(values, grid))
        if not np.isfinite(value) or value < 0: raise ValueError("mean function must be finite and non-negative")
        return value
    ...
    def increment_probability(self, n: int, s: float, t: float) -> float:
        ...
        if mean < -1e-10: raise ValueError("mean function must be non-decreasing")
...
class _DegeneratePoissonProcess:
    rate = 0.0
    def count_probability(self, n: int, t: float) -> float:
        PoissonProcess._validate_time(t); PoissonProcess._validate_count(n)
