"""Discrete-time martingale tools from the MSPRO Chapter 5 course."""
...
class Filtration:
    """Increasing sequence of finite partitions."""
...
class StoppingTime:
    ...
    @classmethod
    def from_values(cls, space: FiniteProbabilitySpace, values: dict, filtration: Filtration) -> "StoppingTime":
    ...
    def add(self, other: "StoppingTime") -> "StoppingTime":
        self._same(other)
        vals = {o: (np.inf if np.isinf(self.values[o]) or np.isinf(other.values[o]) else self.values[o] + other.values[o]) for o in self.space.outcomes}
...
class Martingale:
    """A discrete-time process with conditional-expectation diagnostics."""
...
class StoppedProcess:
    """The stopped process X^T_n = X_{n wedge T}."""
    ...
    def terminal_value(self) -> RandomVariable:
        ...
        values = {o: self.process[int(self.stopping_time.values[o])].values[o] for o in self.process[0].space.outcomes}
