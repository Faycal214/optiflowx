...
def conditional_expectation_given_event(space: FiniteProbabilitySpace, x: RandomVariable, event: Iterable[Hashable]) -> float:
...
def conditional_probability_given_event(space: FiniteProbabilitySpace, event: Iterable[Hashable], conditioning_event: Iterable[Hashable]) -> float:
...
def independent_partitions(space: FiniteProbabilitySpace, first: Partition, second: Partition, *, atol: float = 1e-12) -> bool:
...
def independent_random_variables(space: FiniteProbabilitySpace, first: RandomVariable, second: RandomVariable, *, atol: float = 1e-12) -> bool:
    """Check independence through partitions generated by two variables."""
    ...
    return independent_partitions(space, Partition.generated_by(first), Partition.generated_by(second), atol=atol)


def conditional_characterization_error(space: FiniteProbabilitySpace, x: RandomVariable, partition: Partition) -> float:
    """Maximum error in the finite conditional-expectation characterization."""
    ce = space.conditional_expectation(x, partition)
    error = 0.0
    for block in partition.blocks:
        indicator = space.random_variable([float(o in block) for o in space.outcomes], name="1_A")
        error = max(error, abs((indicator * x).expectation() - (indicator * ce).expectation()))
    return float(error)


def transform_martingale(martingale: Martingale, function: Callable[[float], float], *, name: str | None = None) -> Martingale:
...
def stopped_martingale(martingale: Martingale, stopping_time: StoppingTime) -> Martingale:
    """Return the stopped process as a martingale on the same filtration."""
    return Martingale(martingale.stopped(stopping_time).sequence(), martingale.filtration)
