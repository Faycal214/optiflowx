...
def test_public_eviews_diagnostic_capture_contains_verified_bg_numbers():
    ...
    assert stats["F-statistic"] == 1.516104
    assert stats["Obs*R-squared"] == 3.064110
    assert stats["df"] == 2
    assert fixture["equation"]["nobs"] == 162
...
def test_public_eviews_cointegration_capture_contains_verified_johansen_numbers():
    ...
    assert trace[0]["eigenvalue"] == 0.395431
    assert trace[0]["statistic"] == 49.14152
    assert maxeig[0]["statistic"] == 26.75064
    assert maxeig[0]["critical_5"] == 27.58434
    assert fixture["system"]["selected_rank"]["trace_5"] == 1
    assert fixture["system"]["selected_rank"]["max_eigen_5"] == 0
